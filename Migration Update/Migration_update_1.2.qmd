...
=======
>>>>>>> 45a30ab7ef60fb3a552747e81f8c021ef78919b3
<<<<<<< HEAD

=======
>>>>>>> c4d3ecdd0d94e692f3540c9ec3cf5e9d2dd2e268
<<<<<<< HEAD
=======
>>>>>>> origin/main
>>>>>>> 45a30ab7ef60fb3a552747e81f8c021ef78919b3
...
lt_ts_plot <- ggplot(cbp_hist, aes(x = fiscal_year, y = apps)) +
  ...
  geom_panel_lines()+
  geom_xspline()+
  stat_xspline(geom = "area", fill = "lightblue", alpha = 0.3)
...
month_swbf_plot <- month_swbf %>%
  ...
  stat_xspline(geom = "area", fill = "lightblue", alpha = 0.2) +
  geom_xspline()
...
swb_tsum <- swb %>%
  ...
  filter(
    # Example filters (currently disabled):
...
swb_ts_plot <- swb_tsum %>%
  ...
  theme_usaid_blue(
    ...
    subtitle    = paste("Monthly Encounters |",
...
swb_tree_plot <- swb|> 
  ...
  ggplot(aes(area = share,
   ...
   theme_usaid_blue(base_family = "Gill Sans",
       caption = cap$swb_alt,
       title = paste("Migrants Encountered at the U.S. Southwest Border"
                     #month2_long,year2
                     ),
...
area_swb_pct_plot <- cbpa %>% 
  ...
  geom_line(position = "stack",
            alpha = 1,
            color = "white")+
  theme_usaid_blue(caption = cap$swb_alt, rf = F,
...
cbpa_stream <- cbpa %>% 
  ...
  geom_stream_label(aes(label = citizenship),
                    ...
                    size = 5,
                    color = "black")+
  theme_usaid_blue(caption = paste0(cap$swb_alt,"Areas are smoothed. Right axis labels refer to share of encounters for the most recent date."),
                   rf = F,
                   main_color_1 = main_color_1,
                   base_size = 20,
                   bl = F)+
...
vz_bar_plot <- country_ts_graph("VENEZUELA", "Venezuelan")
ggs(
  plot = vz_bar_plot, 
  category = "encounters", 
  charttype = "sbar",
...
nca_bar_plot <- swb2 |> 
  ...
  theme_usaid_blue(base_family = "Gill Sans", caption = cap$swb_alt, rf = F, bl = F,
                   base_size = 17)+
  scale_y_continuous(labels = ~round(./1e3,0),
                     breaks =  c(
                                 min(swb2$summig),
...
ggplot(aes(x = date, y = encounter_count, 
  ...
  theme_usaid_blue(base_family = "Gill Sans", caption = cap$swb_alt, rf = F, base_size = 17)+
...
nca_pc_plot <- swb %>% 
  ...
  theme_usaid_blue(base_family = "Gill Sans", caption = cap$swb_alt, rf = F, base_size = 18)+
  scale_y_continuous(
...
top5_other_plot <- swb %>% 
  ...
  theme_usaid_blue(base_family = "Gill Sans",base_size = 18,caption = cap$swb_alt, rf = F, bl = F)+
  scale_y_continuous(
...
top5_other_plot <- swb %>% 
  ...
  scale_x_date(breaks = tm_dby(date2,2),
               date_labels = "%b '%y")+
  geom_rangeframe(color = "grey50", sides = "bl")+
  # geom_flag(data = . %>% filter(date == date2),
  #           aes(country = fips),
...
```{r table prep v2}
fy <- 2024
fytd_nums <- swb %>% filter(year >= fy) %>% distinct(date = month(date)) %>% pull(date)
sltj_totals <- swb |>
...
tree_dar_plot<- dar |>
  ...
  guides(fill = "none")+
  labs(title = paste("Migrants Encountered in the Darien Region | CY 2024"
                     #month2_long,year2
                     ))+
...
colu$encounter %>% 
  ...
  arrange(date) %>% 
  mutate(label = case_when(component == "Regular" ~ "Regular\nArrivals",
...
colu_plot <- colugg %>% 
  ...
  geom_xspline()+
  geom_point(data = . %>% filter(date == "2024-07-28"),
  ...
  scale_x_date(breaks = tm_db(month2, n = 3),
               date_labels = "%b '%y",
  ...
  scale_y_continuous(breaks = c(0,2e4,4e4,6e4),
   ...
   geom_text(aes(x = as_date("2024-07-28"), y = 4e4, 
                label = "Venezuelan\nElection"), 
             nudge_x = -5,
...
#geom_line()+
  ...
  labs(title = "Migrants Encountered at the U.S. Southwest Border",
       ...
       y = NULL,
       x = NULL)+
  coord_cartesian(xlim = c(1925,2025), ylim = c(0,2.65e6),
                  clip = "off")+
  theme(
        panel.border = element_rect(size = 10, fill= NA, color = "transparent"),
        plot.title.position = "plot",
...
parole_plot <- swb |>
  ...
  geom_area(aes(x = date, y = value, fill = citizenship),
            alpha = .8,
            color = "black")+
  labs(y = "Monthly Apprehensions <br> *(Thousands)*",
       x = NULL,
