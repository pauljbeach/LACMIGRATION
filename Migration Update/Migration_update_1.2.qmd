...
lt_ts_plot <- ggplot(cbp_hist, aes(x = fiscal_year, y = apps)) +
  ...
  # Y scale settings
  scale_y_continuous(
    breaks = ggthemes::extended_range_breaks(n = 2),
...
cbpa_stream <- cbpa %>%
  ...
  theme_usaid_blue(
    caption = paste0(cap$swb_alt, ". Areas are smoothed. Right axis labels refer to share of encounters for the most recent date."),
    title = "Share of Migrants Apprehended at the U.S. Southwest Border",
    subtitle = paste(
      "Monthly Encounters |",
      month(range(cbpa$date)[1], label = TRUE, abbr = TRUE),
...
nca_pc_plot <- swb %>%
  ...
  theme(plot.subtitle = element_textbox_simple(size = rel(.8))) +
  scale_color_cbp()
ggs(
  plot = nca_pc_plot,
  category = "encounters",
...
top6 <- swb |>
  ...
  slice_max(encounter_count, n = 6) %>%
  pull(citizenship)

top5_other_plot <- swb %>%
  filter(citizenship %in% top6) %>%
  summarise(across(encounter_count, sum),
  ...
  theme_usaid_blue(base_size = 20,
                   ...
                   subtitle = "Top 6 Countries For Encounters Outside of Mexico and Northern Central America",
                   caption = cap$swb_alt) +
  facet_wrap(~citizenship)+
...
us_other5pc_ts <- swb %>%
  ...
  geom_xspline() +
  labs(
    subtitle = "Top 5 Countries Outside of Mexico and Northern Central America <br><br> Monthly Encounters <br> *(Per 1000 Population)*",
    x = NULL,
    title = "Per Capita Migrant Encounters at the U.S. Southwest Border",
    y = NULL,
    fill = "**Nationality:**"
  ...
  scale_x_date(
    breaks = tm_db(month2, n = 1, year0 = 2019),
    date_labels = "%b '%y"
  ) +
  geom_rangeframe(color = "grey50", sides = "bl") +
  # geom_flag(data = . %>% filter(date == date2),
...
chnv_plot <- cbp_all |>
  ...
  theme_usaid_blue(title = "Migrants Encountered At All U.S. Borders",
                   subtitle = "Thousands of Monthly Encounters | CHNV Nationalities",
                   caption = cap$swb_alt,
                   rf = F, bl = F, 
                   base_size = 20) +
  geom_xspline() +
  ...
  scale_color_metro(labels = c("At Ports of Entry",
...
mig_tables <- pmap(
  ...
  ~ mutate(..1, citizenship = ifelse(str_detect(citizenship, "China"), "China", citizenship)) %>%
    ...
    tab_footnote(footnote = paste(
      ...
      "<br><sup>1</sup>percent change of the current fiscal year-to-date compared to the same months during the previous fiscal year"
    ) %>% md()) |>
...
ts_dar_plot <- dar_tsum |>
  ...
  ggplot(aes(x = date, y = encounter_count)) +
  theme_usaid_blue(
    caption = "Data from Gov. of Panama | chart created by USAID/LAC",
